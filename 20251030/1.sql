-- 프로젝트 DB (boardajax) 선택
use boardajax;

-- CODE MASTER
CREATE TABLE CODE_MASTER (
  CODE_SEQ INT PRIMARY KEY, -- 코드 순번
  CODE VARCHAR(11) NOT NULL, -- 코드 값 자체
  CATEGORY VARCHAR(11) , -- 상위 코드 (부모 코드, 계층 구조 연결용)
  COM_KOR VARCHAR(100) , -- 코드명(한글명)
  SUB_COM VARCHAR(200) , -- 코드에 대한 부가 설명
  USE_YN CHAR(2) , -- 사용여부(Y: 사용, N: 미사용)
  REG_DT DATE -- 등록일자
);

ALTER TABLE CODE_MASTER 
MODIFY reg_dt DATE COMMENT '등록일자';

-- USER_TB
CREATE TABLE USER_TB (
  USER_SEQ INT PRIMARY KEY AUTO_INCREMENT, -- 사용자 고유번호
...
select * from USER_TB where USER_ID = "admin1";

DESC ACCOUNT_TB;

-- ACCOUNT_TB  
CREATE TABLE ACCOUNT_TB (
  ACCOUNT_SEQ INT PRIMARY KEY AUTO_INCREMENT, -- 거래 고유번호
  PROFIT_COST VARCHAR(11) DEFAULT NULL, -- 수익/비용 구분 코드 (CODE_MASTER의 CODE)
  BIG_GROUP VARCHAR(11) DEFAULT NULL, -- 대분류 코드 (CODE_MASTER의 CODE)
  MIDDLE_GROUP VARCHAR(11) DEFAULT NULL, -- 중분류 코드 (CODE_MASTER의 CODE)
  SMALL_GROUP VARCHAR(11) DEFAULT NULL, -- 소분류 코드
  DETAIL_GROUP VARCHAR(11) DEFAULT NULL, -- 세부 항목 코드
  COMMENTS VARCHAR(200) DEFAULT NULL, -- 거래 설명 (비용 상세 입력 칸 내용)
  TRANSACTION_MONEY DECIMAL(15,0) DEFAULT NULL, -- 거래 금액
  TRANSACTION_DATE DATE DEFAULT NULL, -- 거래 일자
  WRITER VARCHAR(20) DEFAULT NULL, -- 작성자ID
  REG_DATE DATE -- 등록일자
);
...
SELECT * FROM CODE_MASTER WHERE CATEGORY = 'AA00000';

CREATE OR REPLACE VIEW vw_account_tb AS
SELECT
  a.ACCOUNT_SEQ,
  a.PROFIT_COST,
  pc.COM_KOR AS PROFIT_COST_NM,
  a.BIG_GROUP,
  bg.COM_KOR AS BIG_GROUP_NM,
  a.MIDDLE_GROUP,
  mg.COM_KOR AS MIDDLE_GROUP_NM,
  a.SMALL_GROUP,
  sg.COM_KOR AS SMALL_GROUP_NM,
  a.DETAIL_GROUP,
  dg.COM_KOR AS DETAIL_GROUP_NM,
  a.COMMENTS,
  a.TRANSACTION_MONEY,
  DATE_FORMAT(a.TRANSACTION_DATE, '%Y-%m-%d') AS TRANSACTION_DATE,
  a.WRITER,
  u.USER_NAME AS WRITER_NM,
...
LEFT JOIN USER_TB u ON a.WRITER = u.USER_ID;

SHOW FULL TABLES WHERE TABLE_TYPE = 'VIEW';
SELECT * FROM vw_account_tb;

SELECT * FROM vw_account_tb LIMIT 10;
